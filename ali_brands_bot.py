...
async def process_wrong_custom_case(message: types.Message, state: FSMContext):
    if message.text == "🔙 Orqaga":
        ...
        await message.answer("❌ Iltimos, siz tanlagan chexol rasmi yoki screenshot'ini yuboring!\nQo'llab-quvvatlanadigan formatlar: JPG, PNG, GIF")
...
async def process_confirmation(message: types.Message, state: FSMContext):
    if message.text == "✅ Zakazni tasdiqlash":
        ...
        await message.answer("❌ Zakaz bekor qilindi.\n\n🔄 Yangi zakaz berish uchun /start ni bosing.", reply_markup=ReplyKeyboardRemove())
        await state.clear()
...
# Yakuniy tasdiqlash
@dp.message()
async def handle_final_confirmation(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    ...
    if user_id in temp_orders and message.text in ["✅ Ha, aniq tasdiqlyman", "❌ Yo'q, bekor qilaman"]:
        if message.text == "✅ Ha, aniq tasdiqlyman":
            try:
                ...
                await message.answer(confirmation_text, reply_markup=ReplyKeyboardRemove(), parse_mode="HTML")
                
            except Exception as e:
                logging.error(f"Yakuniy tasdiqlashda xatolik: {e}")
                await message.answer("❌ Xatolik yuz berdi. /start ni bosing.", reply_markup=ReplyKeyboardRemove())
